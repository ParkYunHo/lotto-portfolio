...
import com.john.lotto.job.statics.tasklet.PeriodNumberStaticTasklet
...
class LottoNumberBatchJob (
    ...
    private val periodNumberStaticTasklet: PeriodNumberStaticTasklet,
    ...
    fun lottoNumberJob(jobRepository: JobRepository, transactionManager: JpaTransactionManager): Job =
        JobBuilder("lottoNumberJob", jobRepository)
            ...
            .start(lottoNumberStep(jobRepository, transactionManager))
                .on("*")
                // 기간별 당첨번호 통계
                .to(periodNumberStaticStep(jobRepository, transactionManager))
                // 배치 성공결과 알림전송
                .next(lottoNumberNoticeResultStep(jobRepository, transactionManager))
            .from(lottoNumberStep(jobRepository, transactionManager))
                .on("FAILED")
                // 배치 실패결과 알림전송
                .to(lottoNumberNoticeResultStep(jobRepository, transactionManager))
                .end()
            .build()
    ...
    @Bean
    fun periodNumberStaticStep(jobRepository: JobRepository, transactionManager: PlatformTransactionManager): Step =
        StepBuilder("periodNumberStaticStep", jobRepository)
            .tasklet(periodNumberStaticTasklet, transactionManager)
            .build()
